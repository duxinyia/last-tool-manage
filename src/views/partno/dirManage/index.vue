...
<script setup lang="ts" name="dirManage">
...
import { getAllMatDRIOfBuApi } from '/@/api/partno/dirManage';
// 引入组件
...
const state = reactive<TableDemoState>({
	tableData: {
		...
		header: [
			...
			{ key: 'driName', colWidth: '', title: 'DRI姓名', type: 'text', isCheck: true },
			{ key: 'matCount', colWidth: '', title: '料號數量', type: 'text', isCheck: true },
		],
		// 配置项（必传）
		config: {
			total: 0, // 列表总数
		...
		btnConfig: [
			{ type: 'look', name: '查看料號', color: '#438df5', isSure: false, icon: 'ele-View' },
...
const getTableData = async () => {
	...
	const res = await getAllMatDRIOfBuApi();
